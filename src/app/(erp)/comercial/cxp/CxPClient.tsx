...
export default function CxPClient({ documentos, totalPendiente }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        ...
        <p className="text-gray-500 mt-1">Documentos pendientes de pago</p>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          <p className="text-sm text-gray-500">Total Pendiente</p>
          <p className="text-2xl font-bold font-mono text-red-600">{formatMonto(totalPendiente)}</p>
          <p className="text-xs text-gray-400">{documentos.length} documentos</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          <p className="text-sm text-gray-500">Vencido (&gt;30d)</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        {filtrados.length > 0 && (
          <div className="border-t px-4 py-3 flex justify-between font-bold text-sm">
            <span>Total</span>
            <span className="font-mono text-red-600">{formatMonto(filtrados.reduce((s, d) => s + d.saldo, 0))}</span>
          </div>
        )}
      </div>
    </div>
