"""
Scraping SII - Registro de Compras → Supabase
...
def descargar_periodo(driver, periodo_mes, periodo_anho):
    ...
    time.sleep(3)
    tab = wait.until(EC.presence_of_element_located((By.XPATH, "//strong[text()='COMPRA']")))
...
def esperar_csv(periodo_mes, periodo_anho, timeout=30):
    patron = os.path.join(RUTA_DESCARGAS, f"RCV_COMPRA_REGISTRO_{SII_RUT}_{periodo_anho}{periodo_mes}*.csv")
...
def limpiar_csv_anteriores(periodo_mes, periodo_anho):
    patron = os.path.join(RUTA_DESCARGAS, f"RCV_COMPRA_REGISTRO_{SII_RUT}_{periodo_anho}{periodo_mes}*")
...
def subir_a_supabase(csv_path, periodo_mes, periodo_anho):
    ...
    # Clave de dedup para compras: tipo_dte + rut_emisor + folio
    existing = supabase.table("compras_sii").select("tipo_dte, rut_emisor, folio").execute()
    claves_existentes = {f"{r['tipo_dte']}-{r['rut_emisor']}-{r['folio']}" for r in (existing.data or [])}
    ...
    for _, row in df.iterrows():
        ...
        rut_emisor = str(row.get("RUT Proveedor", "")).strip()
        folio = str(row.get("Folio", "")).strip()
        clave = f"{tipo_dte}-{rut_emisor}-{folio}"
        ...
        try:
            fr_str = str(row.get("Fecha Recepcion", "")).strip()
            if fr_str:
                fecha_recepcion = pd.to_datetime(fr_str, dayfirst=True).strftime("%Y-%m-%d")
        except Exception:
            pass
        ...
        registro = {
            ...
            "monto_iva": float(row.get("Monto IVA Recuperable", row.get("Monto IVA", 0)) or 0),
        ...
        if len(batch) >= 100:
            supabase.table("compras_sii").insert(batch).execute()
            batch = []
